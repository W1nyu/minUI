...
> **처음이라면 [`docs/실행-가이드.html`](docs/실행-가이드.html)을 브라우저로 열 것.**
> 어느 폴더에서 치는지, 터미널을 몇 개 여는지, 안 될 때 무엇을 보는지까지 적어 뒀다.
> Docker 없이 프런트만 띄우는 더 짧은 길도 거기 있다.
...
→ 첫 화면은 **누구로 볼지 고르는 로그인**이다. 고령·중년·청년·상점 12명이 있고,
각자 통장과 거래내역과 자동이체가 다르다.

**비밀번호가 없다.** 여섯 자리 키패드는 은행 앱의 모양을 보여 주려고 둔 시늉이고
**무엇을 누르든 들어간다.** 표에도 서버에도 비교할 값을 두지 않는다 — 값을 적어 두면
그것만으로 화면이 무언가를 지킨다고 말하게 되는데, 이 데모가 지키는 것은 아무것도 없다.
화면 아래에 "비밀번호를 확인하지 않으며"라고 상시로 적어 둔다.

**시연 동선은 이것 하나면 된다.** 김순자로 들어가 박정호에게 3만원을 보내고, 나가서
박정호로 들어가면 **그 3만원이 도착해 있다.** 원장은 하나이고 보는 자리만 사람마다
다르다 — 이 데모가 여태 못 보여 준 것이 그것이다. 이체는 늘 원장 양쪽을 정확히
움직였는데, 받는 쪽을 볼 화면이 없었다.

사람이 바뀌면 **배운 말과 홈 카드 배치도 함께 바뀐다** (`storageKey`가 사람마다 다르다).
한 기기를 나눠 쓸 때 남의 개인화가 내 화면에 뜨면 그 자체가 노출이라서다 (§11.1).

**실계좌·마이데이터는 연결하지 않는다.** 백엔드 없이 정적 데모를 열면 브라우저 탭 안의
가상 원장으로, 위 백엔드를 함께 띄우면 `/mock/openbanking/v2.0/*` Mock endpoint로 동작한다.
두 경우 모두 테스트 계좌 19개와 가상 OAuth 표기만 사용한다. 음성/AI는 메뉴와 입력 제안을
...
**사람 표(`shared/contracts/demo-users.json`)의 `version`이 올라가면 백엔드가 뜰 때
데모 원장을 지우고 다시 깐다.** 로그에 경고로 남는다. 지워지는 것은 이 가상 원장의
계좌·분개·이체 기록뿐이고, 그렇게 하지 않으면 사람을 늘려도 기존 개발 DB에는 영영
안 들어온다.

**사람과 계좌는 브라우저 원장과 Spring 원장이 같은 파일을 읽는다.** 두 곳에 적으면
반드시 어긋나고, 어긋나면 "어디서 봤느냐"에 따라 다른 사람이 나온다 —
`shared/contracts/openbanking-cases.json`을 한 곳에 둔 것과 같은 이유다.
...
- **누구로 볼까요** — 사람군별로 나눈 12명 중 하나를 골라 여섯 자리를 넣는다.
  고른 사람의 통장·거래내역·자동이체·입금예정만 보인다. **시연용이라는 고지가 화면에서
  사라지지 않는다**
- **보낼 통장 고르기** — 통장이 둘 이상인 사람에게만 뜬다. 하나뿐인 사람에게 고르는 칸을
  보이면 이 저장소가 줄이려는 단계 수가 늘어난다
- **전체 계좌 조회** — 내 통장을 고르면 그 통장의 최근 내역이 아래에 붙는다. 잔액 보기와
  나눈 이유는 저쪽이 **답을 읽는** 화면(탭 0회)이고 이쪽이 **고르는** 화면이라서다
