...
`https://w1nyu.github.io/minUI/`는 정적 호스팅이라 `/api/*`가 없다.

| | 공개 배포 | 로컬 전체 구성 |
|---|---|---|
| 기기 안 검색 (n-gram·자모·학습) | ✅ | ✅ |
| 어려운 말 풀이 | ✅ 미리 구워 둔 432개 | ✅ + 없는 것은 그 자리에서 |
| 다음 단계 안내 | ✅ 카탈로그에서 계산 | ✅ |
| 제안 검증 (`validateProposal`) | ✅ | ✅ |
| 가상 이체 (원장이 실제로 바뀜) | ✅ 브라우저 안 | ✅ Spring + Postgres |
| LLM 도우미 (`/api/assist`) | ❌ 되묻기로 내려감 | ✅ |
| 원격 의미 검색 (`/api/match`) | ❌ 꺼짐 | 플래그로 켬 |
| Studio 실시간 수집 | ❌ 3곳 재생 | ✅ Playwright |

**없는 것이 고장으로 보이지 않게 만들어 뒀다.** 도우미가 없으면 `assist`를 아예 안 넘겨
"묻는 중" 상태가 생기지 않고, 되묻기가 그대로 답이 된다 — 그것이 「실패 회복」이다.
